...
local cat, desc = "ojapFridge", "~y~Réfrigérateur du O'Jap"
local sub = function(str)
    return cat .. "_" .. str
end

Onore.netRegisterAndHandle("ojapCbItem", function()
    awaitingServerUpdate = false
end)

Onore.netRegisterAndHandle("ojapOpenFrigo", function(items)
                ...
                for k,v in pairs(items) do
                    RageUI.ButtonWithStyle(("Prendre \"~y~%s~s~\""):format(v.label), "Appuyez pour prendre du stock", {RightBadge = RageUI.BadgeStyle.Franklin}, not awaitingServerUpdate, function(_,_,s)
                        if s then
                            ...
                            OnoreClientUtils.toServer("ojapRequestItem", k)
                        end
                    end)
                end
            end, function()
