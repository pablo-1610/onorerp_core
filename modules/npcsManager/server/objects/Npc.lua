...
--[[
  ...
  File [Npc] created at [17/04/2021 18:47]
...
setmetatable(Npc, {
    __call = function(_, model, ai, frozen, position, animation, restricted, allowed)
        ...
        self.id = #OnoreSNpcsManager.list + 1
        self.model = model
        self.ai = ai
        self.frozen = frozen
        ...
        OnoreSNpcsManager.list[self.id] = self
        return self;
    end
})
...
function Npc:playSpeech(speech, param)
    if self:isRestricted() then
        for source, _ in pairs(self.allowed) do
            OnoreServerUtils.toClient("npcPlaySound", source, self.id, speech, param)
        end
    else
        OnoreServerUtils.toAll("npcPlaySound", self.id, speech, param)
    end
end
