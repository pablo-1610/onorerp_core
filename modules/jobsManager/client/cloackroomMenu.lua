...
--[[
  ...
  File [cloackroomMenu] created at [18/04/2021 18:35]
...
Onore.newWaitingThread(100, function()
    badgeFromValue = {
...
Onore.netRegisterAndHandle("openCloackroom", function(job)
                ...
                RageUI.ButtonWithStyle("Tenue de travail", "Vous permets d'enfiler votre tenue de travail", { RightBadge = badgeFromValue[isWithWorkingClothes] }, true, function(_, _, s)
                    if s and not isWithWorkingClothes then
                        ...
                        TriggerEvent('skinchanger:getSkin', function(skin)
                            TriggerEvent('skinchanger:loadClothes', skin, OnoreSharedCustomJobs[job].clothes[Job.grade_name][byIndex[skin.sex == 0]])
                        end)
                    end
                end)
            end, function()
